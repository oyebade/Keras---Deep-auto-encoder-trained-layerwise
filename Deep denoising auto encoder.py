...
####Note that this program employs denosing criterion only for pre-training (greedy layerwise training)
...
#Corruption of inputs into the first auto encoder module
noise_factor1 = 0.3
X_train_pretrain = X_train + noise_factor1 * np.random.normal(loc=0.0, scale=1.0, size=X_train.shape)
X_train_pretrain = np.clip(X_train_pretrain, 0., 1.)

#############################Auto encoder module 1 goes here
#############################
#############################
...
ae_module1.fit(X_train_pretrain, X_train, nb_epoch=50, batch_size=100, shuffle=True)
...
#Corruption of inputs into the second auto encoder module
noise_factor2 = 0.2
ae_module2_input_pretrain = ae_module2_input + noise_factor2 * np.random.normal(loc=0.0, scale=1.0, size=ae_module2_input.shape)
ae_module2_input_pretrain = np.clip(ae_module2_input_pretrain, 0., 1.)
...
ae_module2.fit(ae_module2_input_pretrain, ae_module2_input, nb_epoch=20, batch_size=100, shuffle=True)
...
#Corruption of inputs into the third auto encoder module
noise_factor3 = 0.1
ae_module3_input_pretrain = ae_module3_input + noise_factor3 * np.random.normal(loc=0.0, scale=1.0, size=ae_module3_input.shape)
ae_module3_input_pretrain = np.clip(ae_module3_input_pretrain, 0., 1.)
...
ae_module3.fit(ae_module3_input_pretrain, ae_module3_input, nb_epoch=20, batch_size=100, shuffle=True)
...
#Corruption of test inputs into the first auto encoder module for visualization of learned filters
X_test_noisy = X_test + noise_factor1 * np.random.normal(loc=0.0, scale=1.0, size=X_test.shape)
X_test_noisy = np.clip(X_test_noisy, 0., 1.)

#Next, let's visualize ten of the input images and the reconstructed outputs from auto encoder module1.
decoded_imgs = ae_module1.predict(X_test_noisy )
n = 10

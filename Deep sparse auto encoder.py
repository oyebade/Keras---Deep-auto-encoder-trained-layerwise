...
####Note that this program employs sparsity criterion only for pre-training (greedy layerwise training)
...
ae_module1.add(Dense(hidd1_units, input_dim=input_units, init='uniform',
                     activity_regularizer=regularizers.activity_l2(1e-7), activation='relu'))
ae_module1.add(Dense(output1_units, activation='sigmoid'))
ae_module1.compile(loss='mse', optimizer='adam', metrics=['accuracy'])
ae_module1.fit(X_train, X_train, nb_epoch=20, batch_size=100, shuffle=True)
...
ae_module2.add(Dense(hidd2_units, input_dim= enc2_input_units, init='uniform',
                     activity_regularizer=regularizers.activity_l2(1e-7), activation='relu'))
ae_module2.add(Dense(output2_units, activation='sigmoid'))
ae_module2.compile(loss='mse', optimizer='adam', metrics=['accuracy'])
ae_module2.fit(ae_module2_input, ae_module2_input, nb_epoch=1, batch_size=100, shuffle=True)
...
ae_module3.add(Dense(hidd3_units, input_dim= enc3_input_units, init='uniform',
                     activity_regularizer=regularizers.activity_l2(1e-7), activation='relu'))
ae_module3.add(Dense(output3_units, activation='sigmoid'))
ae_module3.compile(loss='mse', optimizer='adam', metrics=['accuracy'])
ae_module3.fit(ae_module3_input, ae_module3_input, nb_epoch=1, batch_size=100, shuffle=True)
...
ae_module_ft.add(Dense(nb_classes, activation='softmax', W_regularizer=l2(0.00001)))
sgd = SGD(lr=0.01, decay=1e-6, momentum=0.7, nesterov=True)
ae_module_ft.compile(loss='categorical_crossentropy', optimizer='sgd', metrics=['accuracy'])

ae_module_ft.fit(X_train, Y_train, nb_epoch=1, batch_size=100, shuffle=True)
...
#Next, let's visualize the first ten input images and the reconstructed outputs from auto encoder module1.
